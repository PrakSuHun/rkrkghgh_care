...
※ 판단 근거 작성 규칙 (반드시 준수):
- **모든 섹션의 "opinion" 필드**는 반드시 **"보호자 면담을 통해 판단된 내용으로는"** 으로 시작.
- 그 뒤에 해당 내용을 자연스러운 서술로 이어서 작성한다.
- 예시: "보호자 면담을 통해 판단된 내용으로는 어르신이 보행 시 도움이 필요하심."
- 예시: "보호자 면담을 통해 판단된 내용으로는 최근 식사량이 줄어 영양상태에 주의가 필요함."
- section10_summary 네 개 필드도 동일하게 "보호자 면담을 통해 판단된 내용으로는" 으로 시작.

※ 문체 규칙 (모든 텍스트 필드에 적용):
- **반드시 서술체 경어 사용**: "~이심", "~필요함", "~드심", "~하심", "~됨", "~이 요구됨" 같은 명사형/종결형.
- **구어체 금지**: "~이에요", "~있어요", "~하세요", "~셨어요" 같은 회화체 사용 금지.
- 감탄사·말버릇·반복 표현 제거 ("어~", "그~", "저기" 등).
- 존중·객관 어조 유지. 행정 서식에 어울리는 간결하고 정제된 서술.
